...
angular.module('fancyCombobox', ['angular-groupSort'])
.directive('fancyCombobox', function () {
    return {
        ...
        require: "^groupSort",
        template: '<div class="btn-group dropup" uib-dropdown style="width:250px"> \
...
//                                    <combo-categories ng-repeat="group in groups.objects" category="group"></ul> \
                           </div>',
        restrict: 'E',
        replace: true,
...
.directive('comboCategories', function ($compile, Utilities) {
    return {
        restrict: "E",
        replace: true,
        scope: {
            ...
            element.append('<li style="font-style: italic; font-weight: bold; color: grey">{{category.categoryName}}</li> \
                    <ul style="list-style: none" > \
                        <li role="mewnuitem" ng-repeat="object in category.objects" style="margin-top:5px; margin-left:10px"> \
                            <a href="#" ng-click="clickEvent($event, object.id)"><img style="margin-right:5px" src="{{object.icon}}" height="16" width="16"><span>{{object.name}}</span></a></li> \
                    </ul>');
